...
struct sid_6581_voice va = { 7382, 20 ,  0 | SID_SAWT , 240, 114, 12, 24, &voice_a_osc, & voice_a_env ,0.0f, NULL, NULL};
struct sid_6581_voice vb = { 17382, 40 ,  0 | SID_SAWT , 660, 114, 8, 24, &voice_b_osc, & voice_b_env ,0.0f, NULL, NULL};
struct sid_6581_voice vc = { 7382, 20 ,  0 | SID_SAWT , 240, 114, 12, 24, &voice_c_osc, & voice_c_env ,0.0f, NULL, NULL};
...
SNDFMT sawtooth_fct(int counter){
  SNDFMT data;
  data = (SNDFMT) floor(  255 * ((float)counter/(float)WAVEFORM_LENGTH) );
  printf("sawtooth_fct: %i data:[%i] is : %i\n", counter, counter, data);
  return data;
}
...
void handleKey(SDL_KeyboardEvent key) {
  ...
  case SDLK_4: // note 4 anspielen
	...
	interface_voice->sustain = 6;
	interface_voice->release=6;
	(key.state == SDL_PRESSED) ?  ( interface_voice->control = interface_voice->control | SID_GATE ) : 0 ;
	(key.state == SDL_RELEASED) ? ( interface_voice->control = interface_voice->control & ~SID_GATE ) : 0 ;
...
void init_SID(struct sid_6581* sid){
  ...
  sid->voicea->osc->pulse = gen_pwm_buffer(0.2);
  ...
  sid->voiceb->osc->pulse = gen_pwm_buffer(0.2);
  ...
  sid->voicec->osc->pulse = gen_pwm_buffer(0.2);
